...
glance(mod_all)
```
This isn't a hug number of variables, but there is some correlation, and we might be able to help the model by reducing the number of variables we use.
...
pca_rec <- recipe(Sale_Price ~ ., data=ames_sub) %>%
  ...
  step_pca(
    ...
    num_comp = 7, # number of components to keep, default 5
    )
```
...
Notice that all the variables are now uncorrelated:
